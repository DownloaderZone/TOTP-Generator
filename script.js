...
/***************************************************
 * 3. State, Config & Branding
 ***************************************************/
const BRAND = Object.freeze({
  name: "Downloader Zone",
  copyright: "© 2025 Downloader Zone",
  repoUrl: "https://github.com/DownloaderZone/TOTP-Generator",
  pagesUrl: "https://2fa.downloader.zone/"
});
...
function setAdvancedPanelState(isOpen) {
  isAdvancedOpen = isOpen;
  advancedSettings.style.display = isOpen ? "block" : "none";
  toggleArrow.innerHTML = isOpen ? arrowDownSVG : arrowRightSVG;
  toggleText.textContent = isOpen ? "Hide Advanced Settings" : "Show Advanced Settings";
}
...
function openOrCloseAdvancedPanel() {
  ...
  const isNonDefault =
      parseInt(digitsInput.value, 10) !== DEFAULT_SETTINGS.digits ||
      parseInt(periodInput.value, 10) !== DEFAULT_SETTINGS.period ||
      algorithmSelect.value !== DEFAULT_SETTINGS.algorithm;

  // If user settings differ from defaults, show the advanced panel
  setAdvancedPanelState(isNonDefault);
}
...
toggleAdvanced.addEventListener("click", () => {
  setAdvancedPanelState(!isAdvancedOpen);
});
...
console.log(
  '%cDownloader Zone%c\nTOTP Generator is ready.',
  'color: #fff; background: #0f62fe; padding: 4px 8px; border-radius: 4px; font-size: 16px;margin-top: 15px;',
  'color: #0f62fe; font-size: 14px;margin-bottom: 15px;'
);

// Project links
console.log(
  '%cThe project is 100%% open-source, licensed under GPL-3.0 and deployed on GitHub Pages.' +
  '%c\n\n\nProject: %c' + BRAND.repoUrl +
  '%c\nLive site: %c' + BRAND.pagesUrl +
  '%c\nLicense: %c' + BRAND.repoUrl + '/blob/main/LICENSE',
  'color: #0f62fe; font-size: 14px;margin-top: 15px;',
  'color: #0f62fe; font-style: italic; font-size: 14px;',
